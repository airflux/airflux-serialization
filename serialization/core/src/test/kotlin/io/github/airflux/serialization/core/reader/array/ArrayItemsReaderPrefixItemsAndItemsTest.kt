...
internal class ArrayItemsReaderPrefixItemsAndItemsTest : FreeSpec() {
    ...
    init {
        ...
        "The `JsArrayItemsReader` type for only prefix-items and items" - {

            "when the source is empty" - {
                val source = JsArray()
                val reader = arrayItemsReader(prefixItems = listOf(StringReader), items = StringReader)
                ...
                "when read was some errors" - {
                    ...
                    "when fail-fast is true" - {
                        ...
                        "then reader should return first error" {
                            val reader = arrayItemsReader(prefixItems = listOf(IntReader), items = IntReader)
                    ...
                    "when fail-fast is false" - {
                        ...
                        "then reader should return all errors" {
                            val reader =
                                arrayItemsReader(prefixItems = listOf(IntReader, StringReader), items = IntReader)
