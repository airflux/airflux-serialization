...
class ObjectWriter(private val configuration: ObjectWriterConfiguration) {
    ...
    @AirfluxMarker
    class Builder<T : Any> {
        private val properties = mutableListOf<JsWriterProperty<T, *>>()

        fun <P : Any> requiredProperty(name: String, from: (T) -> P, writer: JsWriter<P>) {
            JsWriterProperty.Required(name, from, writer).also { properties.add(it) }
        }

        fun <P : Any> optionalProperty(name: String, from: (T) -> P?, writer: JsWriter<P>) =
            JsWriterProperty.Optional.Simple(name, from, writer).also { properties.add(it) }

        fun <P : Any> optionalProperty(name: String, from: (T) -> P?, writer: JsArrayWriter<P>) =
            JsWriterProperty.Optional.Array(name, from, writer).also { properties.add(it) }

        fun <P : Any> optionalProperty(name: String, from: (T) -> P?, writer: JsObjectWriter<P>) =
            JsWriterProperty.Optional.Object(name, from, writer).also { properties.add(it) }

        fun <P : Any> nullableProperty(name: String, from: (T) -> P?, writer: JsWriter<P>) =
            JsWriterProperty.Nullable.Simple(name, from, writer).also { properties.add(it) }

        fun <P : Any> nullableProperty(name: String, from: (T) -> P?, writer: JsArrayWriter<P>) =
            JsWriterProperty.Nullable.Array(name, from, writer).also { properties.add(it) }

        fun <P : Any> nullableProperty(name: String, from: (T) -> P?, writer: JsObjectWriter<P>) =
            JsWriterProperty.Nullable.Object(name, from, writer).also { properties.add(it) }
